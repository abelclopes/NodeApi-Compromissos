...
export class CompromissosApi {
     ...
     * @param login 
     */
    public apiCompromissosByCompromissoIdPut(compromisso: models.CompromissosModel, extraHttpRequestParams?: any): Observable<models.CompromissosModel> {
        return this.apiCompromissosByCompromissoIdPutWithHttpInfo(compromisso, extraHttpRequestParams)
            .map((response: Response) => {
                if (response.status === 204) {
                    return undefined;
                } else {
     ...
     * @param login 
     */
    public apiCompromissosByCompromissoIdPutWithHttpInfo(compromisso: models.CompromissosModel, extraHttpRequestParams?: any): Observable<Response> {
        const path = this.Url + '/api/Compromissos/${CompromissoId}'
                    .replace('${' + 'CompromissoId' + '}', String(compromisso._id));

        let queryParameters = new URLSearchParams();
        let headers = new Headers(this.defaultHeaders.toJSON());
        
        if (compromisso._id === null || compromisso._id === undefined) {
            throw new Error('Required parameter CompromissoId was null or undefined when calling apiCompromissosByCompromissoIdPut.');
        }
